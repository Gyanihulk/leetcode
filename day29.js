...
function predictPartyVictory(senate) {
  ...
  while (radiantQueue.length > 0 && direQueue.length > 0) {
    const radiantSani = radiantQueue.shift();
    const direSani = direQueue.shift();

    if (radiantSani < direSani) {
      radiantQueue.push(radiantSani + senate.length);
    } else {
      direQueue.push(direQueue + senate.length);
    }
  }
